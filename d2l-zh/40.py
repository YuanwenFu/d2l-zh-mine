...
def func1():
    ...
    def get_net(devices):
        ...
        #将模型参数分配给用于计算的CPU或者GPU 
        finetune_net = finetune_net.to(devices[0]) 
        #冻结参数 
        for param in finetune_net.features.parameters():
            param.requires_grad = False #不更新特征提取网络的参数 
        return finetune_net 
    ...
    devices, num_epochs, lr, wd = my_d2l.try_all_gpus(), 10, 1e-4, 1e-4 
    lr_period, lr_decay, net = 2, 0.9, get_net(devices) 
    train(net, train_iter, valid_iter, num_epochs, lr, wd, devices, lr_period, lr_decay) 
    #train loss 1.041, valid loss 1.478 
    ...
    num_classes = len(train_valid_ds.classes)
    with open("submission.csv", "w") as f:
        f.write("id," + ",".join(train_valid_ds.classes) + "\n") 
        for i, pred in zip(ids, preds):
            one_hot = [0] * num_classes
            one_hot[int(pred)] = 1
            f.write(i.split(".")[0] + "," + ",".join(str(x) for x in one_hot) + "\n") 
    return 
